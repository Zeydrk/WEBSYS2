...
  try {
    const { customerId, email, passwordHash, role } = req.body;

    if (!customerId || !email || !passwordHash) {
      return res.status(400).json({ message: 'customerId, email and passwordHash are required' });
    }

    const existing = await getDb().Account.findOne({ where: { username: email } });
    if (existing) {
...
  try {
    const { email, passwordHash, role, lastLogin } = req.body;
    const account = await getDb().Account.findByPk(req.params.id);
...
    if (email && email !== account.username) {
      const existing = await getDb().Account.findOne({ where: { username: email } });
      if (existing) {
        return res.status(409).json({ message: 'Email already has an account' });
      }
